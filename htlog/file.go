...
// Init file htlog with json config.
// jsonConfig like:
//	{
...
//  	"permit":"0600"
//	}
func (f *fileHTLog) Init(jsonConfig string) error {
	fmt.Printf("fileHTLog Init:%s\n", jsonConfig)
	if len(jsonConfig) == 0 {
		return nil
	}
...
func (f *fileHTLog) needCreateFresh(size int, day int) bool {
...
// WriteMsg write htlog message into file.
func (f *fileHTLog) LogWrite(when time.Time, msgText interface{}, level int) error {
...
func (f *fileHTLog) createLogFile() (*os.File, error) {
	// Open the log file
...
func (f *fileHTLog) newFile() error {
...
func (f *fileHTLog) lines() (int, error) {
...
func (f *fileHTLog) createFreshFile(logTime time.Time) error {
	// file exists
...
RESTART_htlog:
	...
	startHTLogErr := f.newFile()
	go f.deleteOldLog()

	if startHTLogErr != nil {
		return fmt.Errorf("Rotate StartHTLog: %s", startHTLogErr)
	}
	if err != nil {
...
func (f *fileHTLog) Destroy() {
	f.fileWriter.Close()
}

func init() {
	Register(AdapterFile, &fileHTLog{
		Daily:      true,
		MaxDays:    7,
		Append:     true,
		LogLevel:   LevelDebug,
